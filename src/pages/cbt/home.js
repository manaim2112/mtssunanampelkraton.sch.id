...
import { Button, Card, CardBody, CardFooter, CardHeader, Dialog, DialogBody, DialogFooter, DialogHeader, Input, Typography } from "@material-tailwind/react";
...
import { getResultWIthid, getResultWithUserId, startingCBT } from "../../service/cbt/result";
...
export function HomeCBT() {
            ...
            if(e.code === code) {
                ...
                if(use === -1) {
                    startingCBT({idlist : Number(active), iduser:Number(user.id)}).then(e => {
                        console.log(e)
                        if(e) {
                            getSoalWithIdList(Number(active)).then(er => {
                                window.localStorage.setItem("refresh@"+ user.nisn +"@"+ active, JSON.stringify(er))
                                window.localStorage.setItem("timing@"+ user.nisn + "@" + active, new Date())
                                window.localStorage.setItem("data@"+ user.nisn + "@" + active, JSON.stringify(new Array(er.length).fill(null)))
                                nav("/cbt/start/"+ btoa(JSON.stringify(act) + "@" + new Date()))
                                setLoading(false)
                            })
                        }
                    })
                    ...
                    getSoalWithIdList(Number(active)).then(er => {
                        window.localStorage.setItem("refresh@"+ user.nisn +"@"+ active, JSON.stringify(er))
                        getResultWIthid(result[use].id).then(e => {
                            window.localStorage.setItem("timing@" + user.nisn + "@" + active, e.created_at)
    ...
    return (
        <>
            <div className="flex">
                <div className="p-8 bg-green-100 h-screen sticky top-2 hidden md:inline-block">
